            ...
            while (std::getline(layer_names, layer_filename)) {
                ...
                if (layer_content.contains("do_needs_player")) {
                    if (layer_content["do_needs_player"]) {
                        actions_layers_indxs.push_back(render_order.size() - 1); // !! надо по хорошому исправить, чтобы load_layer_from_json возвращал индекс нового сло€ и использовать его, ато так не robust
                    }
                }
